...
export default function InstructorLoginPage({ navigate }: InstructorLoginPageProps) {
    ...
    <div className="min-h-screen bg-[#fdfcf7] flex items-center justify-center p-4 relative overflow-hidden select-none">
      ...
      <div className="bg-white border border-[#e8e8e3] rounded-[24px] max-w-md w-full p-8 shadow-2xl relative z-10 space-y-6">
        <div className="flex items-center space-x-3 justify-center mb-2">
          <div className="w-10 h-10 rounded-xl bg-[#c8852a] flex items-center justify-center text-lg text-white shadow-sm">🎓</div>
          <div className="text-left">
            <h1 className="font-serif text-lg font-bold text-[#1a1a18] leading-tight">Instructor Portal</h1>
            <p className="font-mono text-[9px] tracking-wider text-[#7a7a72] uppercase leading-none mt-0.5">Classroom & Room Console</p>
          </div>
        </div>

        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            <label className="block text-[10px] font-bold text-[#3d3d38] uppercase mb-1.5 tracking-wider">Instructor Email</label>
            <div className="relative">
              <Mail className="absolute left-3.5 top-3 w-4 h-4 text-[#b0b0a8]" />
              <input 
                type="email"
                required
                placeholder="instructor@factory.com"
                value={email}
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full mt-2 bg-[#c8852a] hover:bg-[#b06818] text-white font-sans font-bold text-xs py-3.5 rounded-full flex items-center justify-center transition-colors shadow-sm cursor-pointer disabled:opacity-50"
          >
            {loading ? 'Authenticating...' : 'Access Instructor Console'}
          </button>
        </form>
